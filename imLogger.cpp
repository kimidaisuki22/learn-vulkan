...
void ImLogger::draw() {
  ...
  if (ImGui::BeginChild("scrolling", ImVec2(0, 0), false,
    ...
    const char *buf_end = buffer_.data() + buffer_.size();
    if (Filter.IsActive()) {
      ...
      for (int line_no = 0; line_no < offsets_.size(); line_no++) {
        const char *line_start = buf + offsets_[line_no];
        const char *line_end = (line_no + 1 < offsets_.size())
                                   ? (buf + offsets_[line_no + 1] - 1)
                                   : buf_end;
...
void ImLogger::add(std::string str) {
  auto old_size = buffer_.size();
  buffer_ += str;

  for (auto new_size = buffer_.size(); old_size < new_size; old_size++) {
    if (buffer_[old_size] == '\n') {
      offsets_.push_back(old_size + 1);
    }
  }
}
void ImLogger::reset() {
